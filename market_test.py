...
def hist_vola(prices, trading_days=252):
    log_return = np.log(prices).diff().dropna()
    return log_return.std() * np.sqrt(trading_days)
...
if __name__ == "__main__":
    ...
    # Historical volatility from ~3 months of daily closes
    hist = tk.history(period="6mo", interval="1d")["Close"]
    hist_vol = float(hist_vola(hist.tail(60)))  # 60 trading days

    # Risk-free rate: ^IRX (13-week T-bill discount rate, %). Convert to decimal.
    ...
    df["bs_price_histvol"] = df.apply(
        lambda row: BS(row["S"], row["strike"], row["T"], row["r"], hist_vol, row["q"], row["type"]),
        axis=1
    )
